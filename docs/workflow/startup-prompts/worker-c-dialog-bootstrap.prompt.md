# Worker C Dialog Bootstrap Prompt

Use the block below as the first message when opening a brand-new dialog for Worker C.
Do not shorten it.
Do not skip the handoff step.

```text
你现在是这个项目的 Worker C。先不要直接开工，先按顺序完整阅读下面文件，并严格服从其要求后再开始判断：

1. /Users/lijiabo/Documents/New project/docs/workflow/teams/engineering/mobile-architecture/workers/worker-c/cleanroom-handoff.prompt.md
