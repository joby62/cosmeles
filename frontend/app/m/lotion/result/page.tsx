...
import {
  ...
  buildLotionReasonLines,
  buildLotionRollbackLine,
  buildLotionSegmentLine,
  buildLotionUsageLine,
  buildLotionWhyNotOthers,
...
export default async function LotionResultPage({
  ...
  const segmentLine = buildLotionSegmentLine(signals);
  const reasons = buildLotionReasonLines(signals);
  const notFor = buildLotionNotForLines(signals);
  const whyNotOthers = buildLotionWhyNotOthers(signals);
  ...
  if (!fallbackMode) {
    try {
      ...
      product = products.find((p) => p.category === "lotion") || null;
    } catch {
      product = null;
    }
  }
        ...
        <div className="flex items-start gap-4">
          <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-2xl bg-black/[0.03]">
            {product ? (
              ...
              <div className="flex h-full items-center justify-center text-[12px] text-black/40">Lotion</div>
            )}
          </div>
          <div>
