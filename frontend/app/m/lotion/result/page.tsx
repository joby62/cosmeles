...
function parseAnswers(raw: Search): Record<string, string> | null {
  const group = getValue(raw, "group");
  const issue = getValue(raw, "issue");
  const scene = getValue(raw, "scene");
  const avoid = getValue(raw, "avoid");
  if (!group || !issue || !scene || !avoid) return null;
  return { group, issue, scene, avoid };
}
    ...
    return (
      <section className="pb-12">
        <article className="rounded-[24px] border border-[#ffb39e]/55 bg-[linear-gradient(180deg,#fff8f4_0%,#fff2ed_100%)] px-5 py-5">
          ...
          <h1 className="mt-2 text-[26px] leading-[1.18] font-semibold tracking-[-0.02em] text-[#452016]">润肤霜推荐暂时不可用</h1>
  ...
  return (
    <section className="pb-12">
      ...
      <div className="mt-8 flex flex-wrap gap-2.5">
        ...
        <Link
          href="/m/compare?category=lotion"
          className="inline-flex h-11 items-center justify-center rounded-full border border-black/15 px-5 text-[15px] font-semibold text-black/80 active:bg-black/[0.03]"
        >
          和我在用的对比
        </Link>
