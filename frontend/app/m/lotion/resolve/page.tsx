...
export default async function LotionResolvePage({
  ...
  const reasonLines = buildLotionReasonLines(signals);
  const rollbackLine = buildLotionRollbackLine(signals);
  const qp = toLotionSearchParams(signals).toString();

  return (
    <section className="pb-8">
      <div className="text-[13px] font-medium text-black/45">润肤霜决策 · 收敛中</div>
