...
export default async function BagPage() {
  ...
  const { BAG_RELEASE_NOTES, BAG_SUPPORT_LINKS, BAG_TRUST_POINTS } = getStorefrontTrustCopy(locale);
  const copy =
    locale === "zh"
      ? {
          eyebrow: "袋中",
          title: "把 shortlist、支持信息和下一步动作放在同一处。",
          summary:
            "婕选的袋中页，不只是暂存商品，也应该把商品详情、对比、探索、配送、退货和支持入口都维持在一步之内。目标很简单：基础信息不隐藏，shortlist 不丢失，在真实 commerce 字段接通前也不伪装成完整购物车。",
          roleEyebrow: "当前角色",
          roleTitle: "在支付开放前，先保证 shortlist 可恢复、可继续判断。",
          nextEyebrow: "还想再看一轮？",
          nextTitle: "在最终留下 shortlist 前，先去测配或对比。",
          nextSummary: "测配负责收窄路线，对比负责并排判断近似候选；袋中则让留下来的商品继续保持可见，直到支付和价格能力逐步接上。",
          nextLinks: ["去测配", "查看已存", "去对比"],
        }
      : {
          eyebrow: "Bag",
          title: "Keep your shortlist, trust details, and next step in one place.",
          summary:
            "Jeslect Bag is where saved products stay close to product detail, compare, learn, shipping, returns, and support. The goal is simple: no hidden basics, no lost shortlist, and no fake checkout signals before real commerce data exists.",
          roleEyebrow: "Current bag role",
          roleTitle: "Keep the shortlist recoverable before checkout goes live.",
          nextEyebrow: "Need another pass first?",
          nextTitle: "Use Match or Compare before you finalize the shortlist.",
          nextSummary:
          ...
          nextLinks: ["Open match", "Open saved", "Run compare"],
        };
  return (
      ...
      <section className="mt-10 grid gap-6 xl:grid-cols-[1.02fr_0.98fr]">
        ...
        <div className="space-y-6">
          <article className="rounded-[32px] border border-black/8 bg-white/92 p-6 shadow-[0_20px_46px_rgba(15,23,42,0.06)]">
            <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-slate-500">{copy.roleEyebrow}</p>
            <h2 className="mt-3 text-[30px] font-semibold tracking-[-0.04em] text-slate-950">{copy.roleTitle}</h2>
          ...
          <article className="rounded-[32px] border border-black/8 bg-[linear-gradient(180deg,#eef6ff_0%,#ffffff_100%)] p-6 shadow-[0_20px_46px_rgba(15,23,42,0.06)]">
            <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-sky-700">{copy.nextEyebrow}</p>
            <h2 className="mt-3 text-[30px] font-semibold tracking-[-0.04em] text-slate-950">{copy.nextTitle}</h2>
            <p className="mt-3 text-[15px] leading-7 text-slate-600">{copy.nextSummary}</p>
