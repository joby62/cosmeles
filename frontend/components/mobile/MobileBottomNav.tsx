...
function NavIcon({ name }: { name: "choose" | "compare" | "wiki" | "features" | "bag" }) {
  if (name === "choose") {
  ...
  if (name === "features") {
    return <span className="m-nav-features-icon" aria-hidden="true" />;
  }
...
export default function MobileBottomNav() {
  ...
  const profileItems = [
    { key: "me" as const, label: "我的", href: "/m/me" },
  ...
  const bagActive = pathname.startsWith("/m/bag");
          ...
          <div className={`m-nav-mode-layer ${profileMode ? "m-nav-mode-active" : "m-nav-mode-hidden"}`}>
            ...
            <div className="m-bottom-dock flex h-[60px] min-w-0 flex-1 items-center rounded-[30px] border border-[color:var(--m-nav-border)] bg-[color:var(--m-nav-bg)] px-1.5 shadow-[0_14px_34px_rgba(0,0,0,0.26)]">
              {profileItems.map((item) => {
                const active = item.key === "me" ? meActive : bagActive;
                return (
                  <Link
                    key={item.key}
                    href={item.href}
                  ...
                  >
                    ...
                    <span className={`text-[13px] leading-none ${active ? "font-semibold" : "font-medium"}`}>{item.label}</span>
                  </Link>
                );
              })}
            </div>
